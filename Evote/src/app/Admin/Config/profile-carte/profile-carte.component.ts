...
import { ProfileCarteService } from './ProfileCarte.service';

@Component({
  selector: 'app-profile-carte',
  templateUrl: './profile-carte.component.html',
  styleUrls: ['./profile-carte.component.css']
})
export class ProfileCarteComponent implements OnInit {

 
  forms = this.formclient.get(230)
  ...
  constructor(
    private service: ProfileCarteService,
  ...
  Edite(row: any) {
    ...
    setTimeout(function () {
      ...
      that.formvalue.setControlValue("prix", row.prix);
      that.formvalue.setControlValue("duree", row.duree);
    }, 1000)
    this.showAdd = false
    this.showUp = true
    this.cansel = true
